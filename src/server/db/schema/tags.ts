...
export const productTags = pgTable(
	'product_tags',
	{
		productId: varchar('product_id', { length: 30 })
			...
			.notNull(),
		tagId: varchar('tag_id', { length: 30 }).references(() => tags.id, { onDelete: 'cascade' }).notNull(),
		...lifecycleDates,
	},
	(table) => ({
		pk: primaryKey({
		...
		productTagIdx: index('product_tags_product_id_tag_id_idx').on(table.productId, table.tagId),
	})
);
